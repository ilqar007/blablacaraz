...
namespace BlaBlaCarAz.DAL.Repositories
{
    public class RepoBase<T> : IDisposable, IRepo<T> where T : EntityBase, new()
    {
        protected readonly BlaBlaCarAzContext Db;
        public RepoBase(BlaBlaCarAzContext db)
        {
            //  Db = new XPayDbContext();
            this.Db = db;
            Table = Db.Set<T>();
        }
        //protected XPayDbContextRepoBase(DbContextOptions<XPayDbContext> options)
        //{
        //    //  Db = new XPayDbContext(options);
        //    Table = Db.Set<T>();
        //}

        protected DbSet<T> Table;
      //  protected DbQuery<T> Procedures;

        public BlaBlaCarAzContext Context => Db;
        ...
        public async virtual Task<int> AddAsync(T entity, bool persist = true)
        {
            Table.Add(entity);
            return persist ? await SaveChangesAsync() : 0;
        }
        public async virtual Task<int> AddRangeAsync(IEnumerable<T> entities, bool persist = true)
        {
            Table.AddRange(entities);
            return persist ? await SaveChangesAsync() : 0;
        }
        public async virtual Task<int> UpdateAsync(T entity, bool persist = true)
        {
            Table.Update(entity);
            return persist ? await SaveChangesAsync() : 0;
        }
        public async virtual Task<int> UpdateRangeAsync(IEnumerable<T> entities, bool persist = true)
        {
            Table.UpdateRange(entities);
            return persist ? await SaveChangesAsync() : 0;
        }
        public async virtual Task<int> DeleteAsync(T entity, bool persist = true)
        {
            Table.Remove(entity);
            return persist ? await SaveChangesAsync() : 0;
        }
        public async virtual Task<int> DeleteRangeAsync(IEnumerable<T> entities, bool persist = true)
        {
